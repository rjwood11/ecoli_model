...
p<-leaflet() %>% 
  ...
  setView(lng = -86.791425, lat = 36.08, zoom = 10) %>% 
  addCircleMarkers(data = Forecast, popup = ~popup, color=~cfac(Forecast$Status)) %>%
  addWMSTiles(
...
t<-c("Location","Date","Water Temperature (C)","pH","Precipitation (in)","River Flow (cfs)","Water Height (ft)","Dissolved Oxygen (mg/L)", "E. coli (MPN/100 mL)", "Status")

names(Forecast)[c(2,4:5,7,11,13,15,19,22,23)]<-t


library(formattable)

formattable(Forecast[c(2,4:5,7,11,13,15,19,22,23)], list(
  "River Flow (cfs)" = color_tile("white", "blue"),
  Status = formatter("span", style = x ~ ifelse(x == "Safe",
    formattable::style(color = "green", font.weight = "bold"), formattable::style(color = "orange"))),
  "E. coli (MPN/100 mL)" = color_tile("white", "red")
))


#knitr::kable(Forecast[c(2,4:5,7,11,13,15,19,22,23)], caption = "Sampling Locations and Data", align = "llccccccrr")
