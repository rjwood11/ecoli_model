...
p<-leaflet() %>% 
  ...
  setView(lng = -86.791425, lat = 36.08, zoom = 10) %>% 
  addCircleMarkers(data = Forecast, popup = ~popup, color=~cfac(Forecast$Status),fill=T) %>%
  addWMSTiles(
...
Forecast2$Date<-format(as.Date(Forecast2$Date, format="%Y-%m-%d"), "%B %d %Y")


k<-formattable(Forecast2[c(2,4:5,7,11,13,15,17,19,24:25)], list(
...
Forecast2[c(2,4:5,7,11,13,15,17,19,24:25)] %>%
  ...
  column_spec(11, color = spec_color(Forecast2$`E. coli (MPN/100 mL)`[1:8])) %>%
  column_spec(12,bold=T)



Forecast2[c(2,4:5,7,11,13,15,17,19,24:25,26)] %>%
  kbl(caption = "Sampling Locations and Results",booktabs = T) %>%
  kable_classic("striped" ,full_width = T, html_font = "Cambria") %>%
  #kable_styling(bootstrap_options = c("striped","condensed"),full_width=F,position = "left", 
  #              font_size = 12, html_font = "Cambria") %>%
  column_spec(13, image = spec_pointrange(
