...
cfac<-colorFactor(c("green","yellow","red"), levels=c("Safe","Caution","Warning"),na.color = "grey")
...
p<-leaflet() %>% 
  ...
  setView(lng = -86.791425, lat = 36.08, zoom = 10) %>% 
  addCircleMarkers(data = Forecast, popup = popup, color=~cfac(as.factor(Forecast$Status))) %>%
  addWMSTiles(
